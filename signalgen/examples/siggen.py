...
class SignalGenerator(object):
    ...
    def __init__(self, addr=DEFAULT_ADDRESS, min_output=None, max_output=None, gain_file=None):
        ...
        if self.gain_file:
            panels, gains = np.loadtxt(self.gain_file, unpack=True, usecols=[0,1])
    ...
    def profile_gain(self, filename, real_power, points=91, runs=1):
        """
        ...
        runs : int, optional
            number of runs to take
        """
        assert self.gain_file is None
        outputs = np.linspace(self.min_output, self.max_output, num=points, endpoint=True)
        gains = np.zeros((runs, outputs.size), dtype=np.float)

        for i in range(runs):
            print("\nRun {0}:".format(i + 1))
            counter = iter(range(outputs.size))
            self.power_sweep(outputs, profile_gain_callback, (real_power, gains[i], counter))
        
        means = gains.mean(axis=0)
        stds = gains.std(axis=0)
        with open(filename, 'w+') as gain_file:
            for output, mean, std in zip(outputs, means, stds):
                gain_file.write("{0:>10f} {1:>10f} {2:>10f}\n".format(output, mean, std))

        
def profile_gain_callback(panel_out, state):
    real_power, gains, counter = state
    gain = real_power() - panel_out
    gains[next(counter)] = gain
    print("Panel output: {0:>.2f}, Gain: {1:>.2f}".format(panel_out, gain))
